...
**実機もフラッシュも要らず、devcontainer内で完結します。** ESP-IDFのlinuxターゲットで
ビルドし、同梱のUnityで実行します。
...
| テスト対象 | 主に見ているもの |
|---|---|
| `main/text_util.cpp` | 全角→半角、半角カナ→全角(濁点の合成)、前後の空白除去、UTF-8の1文字読み |
| `main/time_util.h` | ISO8601のパース(`+09:00` / `Z` / 小数秒 / 空白区切り)、日付・時刻の整形 |
| `main/schedule.cpp` | 期間の重なり判定(境界を含むか)と開始時刻順の並び |
| `main/json_parser.cpp` | 実スキーマからの取り込み、除外条件、`bodyPreview`の整形、壊れた入力の扱い |

**LCD・Wi-Fi・タッチに依存するコードは対象外です**(ホストでは動かせないため、実機で
確認します)。テスト本体は`test/host/main/`にあり、アプリのソースはコピーせずに
そのままコンパイルしています。`json_parser`のテストにはサーバの実スキーマを入れて
ありますが、**氏名は架空のものへ差し替えてあります。**
