...
**ON/OFFと輝度は別の操作で、互いに触らない。** ON/OFFはIO2(DISP、`0x03`のbit2)の
1ビットだけ、輝度は`0x05`だけを使う。公式サンプルも`rgb_lcd_port.c`の
`bl_on()`/`bl_off()`と`io_extension.c`の`IO_EXTENSION_Pwm_Output()`に分かれている
(出典: https://github.com/waveshareteam/ESP32-S3-Touch-LCD-7B/)。

**PWMはactive-low**(書く値が大きいほど暗い)。公式が値を97%でクランプする理由を
「完全消灯を防ぐ」とコメントしていること、輝度設定のサンプルに
`assuming active-low configuration`とあることが根拠。

**消灯から復帰するとき、IO2をHighに戻した後にPWM(`0x05`)を書いてはならない。**
書くとバックライトが戻らない(2026-09-14に実機で確認)。**この失敗はI2Cの戻り値では
検出できない。** `0x03`も`0x05`も`ESP_OK`を返すのに、バックライトだけが点かない。
公式サンプルには`bl_off()`を呼ぶ例が無く、「消灯して後で復帰する」使い方には前例が無い。
CH32V003のファームウェアのソースは公開されておらず、IO2のGPIO出力とPWM出力が内部で
排他かどうかは判定できないため、**理由は解明していない**。

出力値(`0x03`)はシャドウを持って8bitまとめて書くため、**用途のビット以外を
意図せず動かさないこと**が重要になる。公式のシャドウ初期値は`0xFF`だが、
このプロジェクトでは`bit5`(USB_SEL)だけをLowにした`0xDF`を基準にしている。
...
2. **`USB_SEL`(bit5)は常にLow。** HighにするとネイティブUSBがCAN側へ切り替わり、
